CREATE DATABASE retoree DEFAULT CHARACTER SET UTF8 COLLATE UTF8_GENERAL_CI;


CREATE TABLE ADMIN
(
  ADMIN_ID VARCHAR(100) NOT NULL COMMENT '관리자정보  UID',
...
(
  ...
  CAR_ID     VARCHAR(200) NOT NULL COMMENT '차량 정보  UID',
...
(
  CAR_IMG_ID     VARCHAR(200) NOT NULL COMMENT '차량 정보  UID',
  ATTACHFILE_SEQ VARCHAR(200) NOT NULL COMMENT '첨부파일 일련번호',
  IMG_INFO       VARCHAR(200) NULL     COMMENT '이미지 정보',
...
(
  CAR_ID         VARCHAR(200) NOT NULL COMMENT '차량 정보  UID',
  ORIGIN         VARCHAR(100) NULL     COMMENT '국산/수입',
  ...
  ADDR_CITY      VARCHAR(100) NULL     COMMENT '판매지역',
...
(
  ...
  WHEEL_HOUSE  VARCHAR(100) NULL     COMMENT '휠 하우스',
...
(
  ATTACHFILE_SEQ    VARCHAR(200) NOT NULL COMMENT '첨부파일 일련번호',
  SOURCE_UNIQUE_SEQ VARCHAR(200) NOT NULL COMMENT '원본레코드 UNIQUE SEQ',
  ATTACHFILE_TYPE   VARCHAR(200) NULL     COMMENT '첨부파일 유형',
  ORIGINALFILE_NAME  VARCHAR(200) NOT NULL COMMENT '원본 파일명',
...
(
  ...
  NOTICE_TOP     VARCHAR(45)  NULL     COMMENT '공지사항지정',
  ADMIN_ID       VARCHAR(100) NOT NULL COMMENT '관리자정보  UID',
...
(
  ...
  RVS_TIME       VARCHAR(100) NULL     COMMENT '예약시간(오전/오후)',
  RVS_ADDR       VARCHAR(100) NULL     COMMENT '방문주소',
...
(
  ...
  SALE_DATE VARCHAR(100) NULL     COMMENT '등록일',
  PRIMARY KEY (STATUS_ID)
) COMMENT '판매상태';
...
ALTER TABLE DEALER_INFO
  ADD CONSTRAINT FK_USER_INFO_TO_DEALER_INFO
    FOREIGN KEY (DEALER_ID)
    REFERENCES USER_INFO (USER_ID);

ALTER TABLE ADMIN
  ADD CONSTRAINT FK_USER_INFO_TO_ADMIN
    FOREIGN KEY (ADMIN_ID)
    REFERENCES USER_INFO (USER_ID);

ALTER TABLE USER
  ADD CONSTRAINT FK_USER_INFO_TO_USER
    FOREIGN KEY (USER_ID)
    REFERENCES USER_INFO (USER_ID);
...
ALTER TABLE CS_QNA
  ADD CONSTRAINT FK_USER_TO_CS_QNA
    FOREIGN KEY (USER_ID)
    REFERENCES USER (USER_ID);
...
ALTER TABLE CAR_DETAILE_INFO
  ADD CONSTRAINT FK_DEALER_INFO_TO_CAR_DETAILE_INFO
    FOREIGN KEY (DEALER_ID)
    REFERENCES DEALER_INFO (DEALER_ID);

ALTER TABLE CONTACT
  ADD CONSTRAINT FK_DEALER_INFO_TO_CONTACT
    FOREIGN KEY (DEALER_ID)
    REFERENCES DEALER_INFO (DEALER_ID);

ALTER TABLE EVENT
  ADD CONSTRAINT FK_ADMIN_TO_EVENT
    FOREIGN KEY (ADMIN_ID)
...
ALTER TABLE CS_FNA
  ADD CONSTRAINT FK_ADMIN_TO_CS_FNA
    FOREIGN KEY (ADMIN_ID)
    REFERENCES ADMIN (ADMIN_ID);

ALTER TABLE CS_QNA
  ADD CONSTRAINT FK_ADMIN_TO_CS_QNA
    FOREIGN KEY (ADMIN_ID)
    REFERENCES ADMIN (ADMIN_ID);

ALTER TABLE CAR_IMG
  ADD CONSTRAINT FK_CAR_INFO_TO_CAR_IMG
    FOREIGN KEY (CAR_IMG_ID)
    REFERENCES CAR_INFO (CAR_ID);

ALTER TABLE CAR_IMG
  ADD CONSTRAINT FK_CIP_ATTACHFILE_TO_CAR_IMG
    FOREIGN KEY (ATTACHFILE_SEQ)
    REFERENCES CIP_ATTACHFILE (ATTACHFILE_SEQ);

ALTER TABLE DEALER_INFO
  ADD CONSTRAINT FK_CIP_ATTACHFILE_TO_DEALER_INFO
    FOREIGN KEY (ATTACHFILE_SEQ)
    REFERENCES CIP_ATTACHFILE (ATTACHFILE_SEQ);
