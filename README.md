...
홈페이지들을 보면 사용자의 비밀번호 확인 후 탈퇴 요청을 처리합니다. 이러한 과정에서 암호화가 되어있는 비밀번호를 어떻게 매치시킬지 고민이 많았습니다. PasswordEncoder.matches() 메서드는 Spring Security의 인터페이스인 PasswordEncoder의 메서드 중 하나입니다. 이 메서드는 가입한 비밀번호와 암호화된 비밀번호가 일치하는지를 확인하는 역할을 합니다.

matches() 메서드를 활용하여 입력한 평문 비밀번호와 암호화된 비밀번호가 일치하면 true를 반환하고, 일치하지 않으면 false를 반환하였습니다. 그리고 일치할 때 SecurityContextHolder.clearContext() 메서드를 활용하여 회원의 데이터를 삭제하였습니다.
처음에는 Spring Security에 대해서 모르는 부분이 너무나도 많았지만 파이널 프로젝트를 통해 원리를 조금이나마 이해하게 된 계기가 되었습니다.
